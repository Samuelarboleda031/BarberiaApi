...
namespace BarberiaApi.Controllers
{
    ...
    public class RolesController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] CreateRoleInput input)
        { var r = await _rolService.CreateAsync(input); return r.Success ? Ok(r.Data) : StatusCode(r.StatusCode, r.Error); }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] RoleInput input)
        { var r = await _rolService.UpdateAsync(id, input); return r.Success ? Ok(r.Data) : r.StatusCode == 404 ? NotFound() : BadRequest(r.Error); }
